...
internal class FrontlineArenaFilter : DataFilter {
    ...
    public FrontlineArenaFilter() { }

    internal FrontlineArenaFilter(Plugin plugin, Func<Task> action, FrontlineArenaFilter? filter = null) : base(plugin, action) {
        FilterState = new();
        _range = new() {
            "All",
        };
        var allMaps = Enum.GetValues(typeof(FrontlineMap)).Cast<FrontlineMap>();
        foreach(var map in allMaps) {
            FilterState.Add(map, true);
            _range.Add(MatchHelper.GetFrontlineArenaName(map));
        }
        CurrentIndex = 0;
        ...
        if(ImGui.Combo($"##flMapRangeCombo", ref currentIndex, _range.ToArray(), _range.Count)) {
            _plugin!.DataQueue.QueueDataOperation(async () => {
                ...
                if(currentIndex == 0) {
                    ...
                    //cast as int
                    FrontlineMap selectedMap = (FrontlineMap)currentIndex - 1;
                    foreach(var item in FilterState) {
